...
using ExtKeyAccountMap = std::map<CKeyID, CExtKeyAccount*>;
using ExtKeyMap = std::map<CKeyID, CStoredExtKey*>;
...
public:
    ...
    CWallet(interfaces::Chain& chain, const WalletLocation& location, std::unique_ptr<WalletDatabase> database) : nStealth{0}, nFoundStealth{0}, stealthAddresses{}, mapStealthKeyMeta{}, mapExtAccounts{}, mapExtKeys{}, m_chain(chain), m_location(location), database(std::move(database))
    {
    }
    ...
    int ExtKeyAppendToPack(CExtKeyAccount *sea, const CKeyID &idKey, CEKASCKey &asck, bool &fUpdateAcc) const;
    bool FindStealthTransactions(const CTransaction& tx, mapValue_t& mapNarr);

    void MarkDirty();
    bool AddToWallet(const CWalletTx& wtxIn, bool fFlushOnClose=true);
    void LoadToWallet(const CWalletTx& wtxIn) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool UpdateAnonTransaction(const CTransaction& tx, const uint256& blockHash) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
    bool ProcessAnonTransaction(const CTransaction& tx, const uint256& blockHash, bool& fIsMine, mapValue_t& mapNarr) EXCLUSIVE_LOCKS_REQUIRED(cs_wallet);
     ...
     * @note nChangePos is out-only parameter
     */
    bool CreateTransaction(interfaces::Chain::Lock& locked_chain, const std::vector<CRecipient>& vecSend, CTransactionRef& tx, CReserveKey& reservekey, CAmount& nFeeRet, int& nChangePos,

...
static void WaitForShutdown()
{
    while (!ShutdownRequested())
    {
        ...
        // TokenPay: if shutdown was not requested but TorMgr isn't running, then the Tor thread has exited with error
        //
        if (false == TorMgr::GetInstance().isRunning())
        {
            LogPrintf("Tor stopped unexpectedly, shutting down...\n");
            StartShutdown();
        }
    }
    Interrupt();

...
static UniValue settxfee(const JSONRPCRequest& request)
{
    ...
    if (tx_fee_rate == 0) {
        // automatic selection
    } else if (tx_fee_rate < MIN_TX_FEE)
    {
        throw JSONRPCError(RPC_INVALID_PARAMETER ,
                           strprintf("txfee cannot be less than min relay tx fee (%d)" , MIN_TX_FEE));
    }
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 3)
        throw std::runtime_error(
            RPCHelpMan{"fundrawtransaction",
                ...
                {
                    {"hexstring", RPCArg::Type::STR_HEX,  false,  "", "The hex string of the raw transaction"},
                    {"options", RPCArg::Type::OBJ,  true,  "null", "for backward compatibility: passing in a true instead of an object will result in {\"includeWatching\":true}",
                        {
                            {"changeAddress", RPCArg::Type::STR,  true,  "pool address", "The bitcoin address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM,  true,  "random", "The index of the change output"},
                            {"change_type", RPCArg::Type::STR,  true,  "set by -changetype", "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
                            {"includeWatching", RPCArg::Type::BOOL,  true,  "false", "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL,  true,  "false", "Lock selected unspent outputs"},
                            {"feeRate", RPCArg::Type::AMOUNT,  true,  "not set: makes wallet determine the fee", "Set a specific fee rate in " + CURRENCY_UNIT + "/kB"},
                            {"subtractFeeFromOutputs", RPCArg::Type::ARR,  true,  "empty array", "A json array of integers.\n"
                            "                              The fee will be equally deducted from the amount of each specified output.\n"
                            "                              Those recipients will receive less bitcoins than you enter in their corresponding amount field.\n"
                            "                              If no outputs are specified here, the sender pays the fee.",
                                {
                                    {"vout_index", RPCArg::Type::NUM,  true,  "", "The zero-based output index, before a change output is added."},
                                },
                            },
                            {"replaceable", RPCArg::Type::BOOL,  true,  "fallback to wallet's default", "Marks this transaction as BIP125 replaceable.\n"
                            "                              Allows this transaction to be replaced by a transaction with higher fees"},
                            {"conf_target", RPCArg::Type::NUM,  true,  "fallback to wallet's default", "Confirmation target (in blocks)"},
                            {"estimate_mode", RPCArg::Type::STR,  true,  "UNSET", "The fee estimate mode, must be one of:\n"
                            "         \"UNSET\"\n"
                            "         \"ECONOMICAL\"\n"
                            "         \"CONSERVATIVE\""},
                        },
                        "options"},
                    {"iswitness", RPCArg::Type::BOOL,  true,  "depends on heuristic tests", "Whether the transaction hex is a serialized witness transaction \n"
    ...
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
        throw std::runtime_error(
            RPCHelpMan{"bumpfee",
                ...
                {
                    {"txid", RPCArg::Type::STR_HEX, false, "", "The txid to be bumped"},
                    {"options", RPCArg::Type::OBJ, true, "null", "",
                        {
                            {"confTarget", RPCArg::Type::NUM, true, "fallback to wallet's default", "Confirmation target (in blocks)"},
                            {"totalFee", RPCArg::Type::NUM, true, "fallback to 'confTarget'", "Total fee (NOT feerate) to pay, in satoshis.\n"
            "                         In rare cases, the actual fee paid might be slightly higher than the specified\n"
            "                         totalFee if the tx change output has to be removed because it is too close to\n"
            "                         the dust threshold."},
                            {"replaceable", RPCArg::Type::BOOL, true, "true", "Whether the new transaction should still be\n"
...
public:
    ...
    UniValue operator()(const CStealthAddress& stealthAddress) const { return UniValue(UniValue::VOBJ); }
    UniValue operator()(const CExtKeyPair& extKeyPair) const { return UniValue(UniValue::VOBJ); }
    ...
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 5)
        throw std::runtime_error(
            RPCHelpMan{"walletcreatefundedpsbt",
                ...
                {
                    {"inputs", RPCArg::Type::ARR,  false,  "", "A json array of json objects",
                        {
                            {"", RPCArg::Type::OBJ,  false,  "", "",
                                {
                                    {"txid", RPCArg::Type::STR_HEX,  false,  "", "The transaction id"},
                                    {"vout", RPCArg::Type::NUM,  false,  "", "The output number"},
                                    {"sequence", RPCArg::Type::NUM,  false,  "", "The sequence number"},
                                },
                            },
                        },
                        },
                    {"outputs", RPCArg::Type::ARR,  false,  "", "a json array with outputs (key-value pairs), where none of the keys are duplicated.\n"
                            "That is, each address can only appear once and there can only be one 'data' object.\n"
                            "For compatibility reasons, a dictionary, which holds the key-value pairs directly, is also\n"
                            "                             accepted as second parameter.",
                        {
                            {"", RPCArg::Type::OBJ,  true,  "", "",
                                {
                                    {"address", RPCArg::Type::AMOUNT,  false,  "", "A key-value pair. The key (string) is the bitcoin address, the value (float or string) is the amount in " + CURRENCY_UNIT + ""},
                                },
                                },
                            {"", RPCArg::Type::OBJ,  true,  "", "",
                                {
                                    {"data", RPCArg::Type::STR_HEX,  false,  "", "A key-value pair. The key must be \"data\", the value is hex-encoded data"},
                                },
                            },
                        },
                    },
                    {"locktime", RPCArg::Type::NUM,  true,  "0", "Raw locktime. Non-0 value also locktime-activates inputs"},
                    {"options", RPCArg::Type::OBJ,  true,  "null", "",
                        {
                            {"changeAddress", RPCArg::Type::STR_HEX,  true,  "pool address", "The bitcoin address to receive the change"},
                            {"changePosition", RPCArg::Type::NUM,  true,  "random", "The index of the change output"},
                            {"change_type", RPCArg::Type::STR,  true,  "set by -changetype", "The output type to use. Only valid if changeAddress is not specified. Options are \"legacy\", \"p2sh-segwit\", and \"bech32\"."},
                            {"includeWatching", RPCArg::Type::BOOL,  true,  "false", "Also select inputs which are watch only"},
                            {"lockUnspents", RPCArg::Type::BOOL,  true,  "false", "Lock selected unspent outputs"},
                            ...
                            {"replaceable", RPCArg::Type::BOOL,  true,  "false", "Marks this transaction as BIP125 replaceable.\n"
                            "                              Allows this transaction to be replaced by a transaction with higher fees"},
                            {"conf_target", RPCArg::Type::NUM,  true,  "Fallback to wallet's confirmation target", "Confirmation target (in blocks)"},
                            {"estimate_mode", RPCArg::Type::STR,  true,  "UNSET", "The fee estimate mode, must be one of:\n"
                            "         \"UNSET\"\n"
                            "         \"ECONOMICAL\"\n"
                            "         \"CONSERVATIVE\""},
                        ...
                        "options"},
                    {"bip32derivs", RPCArg::Type::BOOL,  true,  "false", "If true, includes the BIP 32 derivation paths for public keys if we know them"},
                },
                RPCResult{
                            "{\n"

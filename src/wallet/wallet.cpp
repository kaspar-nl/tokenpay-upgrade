...
static int GetBlockHeightFromHash(const uint256& blockHash)
{
    ...
    auto blockHashFindItr = mapBlockIndex.find(blockHash);
    if (mapBlockIndex.cend() == blockHashFindItr)
    {
        return 0;
    }

    return blockHashFindItr->second->nHeight;
}

static int IsAnonCoinCompromised(CPubKey &pubKey, CAnonOutput &ao, ec_point &vchSpentImage)
{
    ...
    RingSignatureMgr::GetInstance().getOldKeyImage(pubKey, pkImage);

    if (vchSpentImage == pkImage || GetKeyImage(*pblocktree, pkImage, kis, fInMempool))
    {
        ao.nCompromised = 1;
        pblocktree->WriteAnonOutput(pubKey, ao);
        if(fDebugRingSig)
...
bool CWallet::UpdateAnonTransaction(const CTransaction& tx, const uint256& blockHash)
{
    assert(tx.IsAnon());
    ...
    for (uint32_t i = 0; i < tx.vin.size(); ++i)
    {
        ...
        if (false == pblocktree->ReadAnonOutput(pkRingCoin, ao))
        {
        ...
        if (false == pblocktree->WriteKeyImage(vchImage, spentKeyImage))
        {
    ...
    for (uint32_t i = 0; i < tx.vout.size(); ++i)
    {
        ...
        CAnonOutput ao;
        if (false == pblocktree->ReadAnonOutput(pkCoin, ao))
        {
        ...
        if (false == pblocktree->WriteAnonOutput(pkCoin, ao))
        {
            LogPrintf("ReadAnonOutput %d failed.\n", i);
            return false;
        }
...
bool CWallet::ProcessAnonTransaction(const CTransaction& tx, const uint256& blockHash, bool& fIsMine, mapValue_t& mapNarr)
{
    uint256 txnHash = tx.GetHash();
    ...
    for (uint32_t i = 0; i < tx.vin.size(); ++i)
    {
        ...
        bool fInMempool;
        if (GetKeyImage(*pblocktree, vchImage, spentKeyImage, fInMempool))
        {
            if (spentKeyImage.txnHash == txnHash && spentKeyImage.inputNo == i)
            {
                if (fDebugRingSig)
                {
                ...
                return UpdateAnonTransaction(tx, blockHash);
            }

            if (TxnHashInSystem(*pblocktree, spentKeyImage.txnHash))
            {
        ...
        WalletBatch wdb{*database};
        COwnedAnonOutput oao;
        ec_point vchNewImage;
        if (!wdb.ReadOldOutputLink(vchImage, vchNewImage))
            vchNewImage = vchImage;

        if (wdb.ReadOwnedAnonOutput(vchNewImage, oao))
        {
            if (fDebugRingSig)
                LogPrintf("%s: input %d keyimage %s found in wallet (owned).\n", __func__, i, HexStr(vchImage).c_str());

            WalletTxMap::iterator mi = mapWallet.find(oao.r_outpoint.hash);
            if (mi == mapWallet.end())
                return error("%s: Error input %d prev txn not in mapwallet %s.", __func__, i, oao.r_outpoint.hash.ToString().c_str());

            CWalletTx& inTx = (*mi).second;
            if (oao.r_outpoint.n >= inTx.tx->vout.size())
            {
                return error("%s: bad wtx %s.", __func__, oao.r_outpoint.hash.ToString().c_str());
            }
            else
            {
                const Coin& coin = pcoinsTip->AccessCoin(oao.r_outpoint);
                if (!coin.IsSpent())
                {
                    LogPrintf("%s: found spent coin %s.\n", __func__, oao.r_outpoint.hash.ToString().c_str());

                    pcoinsTip->SpendCoin(oao.r_outpoint);
                    if (!wdb.WriteTx(inTx))
                    {
            ...
            oao.r_spent = true;
            if (!wdb.WriteOwnedAnonOutput(vchNewImage, oao))
            {
        ...
        int nRingSize = txin.ExtractRingSize();
        if (nRingSize < (Params().GetConsensus().IsProtocolV3(pindexBestHeader ? pindexBestHeader->nHeight : 0) ? 1 : (int)MIN_RING_SIZE)
        ...
        for (uint32_t ri = 0; ri < (uint32_t)nRingSize; ++ri)
        {
            ...
            if (!pblocktree->ReadAnonOutput(pkRingCoin, ao))
                return error("%s: Input %u AnonOutput %s not found, rsType: %d.", __func__, i, HexStr(pkRingCoin).c_str(), rsType);

            if (IsAnonCoinCompromised(pkRingCoin, ao, vchImage) && Params().GetConsensus().IsProtocolV3(pindexBestHeader ? pindexBestHeader->nHeight : 0))
            ...
            if (ao.nBlockHeight == 0 ||
                (pindexBestHeader ? pindexBestHeader->nHeight : 0) - ao.nBlockHeight < MIN_ANON_SPEND_DEPTH)
                return error("%s: Input %u ring coin %u depth < MIN_ANON_SPEND_DEPTH.", __func__, i, ri);

            if (nRingSize == 1)
            {
                ao.nCompromised = 1;
                if (!pblocktree->WriteAnonOutput(pkRingCoin, ao))
        ...
        if (false == blockHash.IsNull())
        {
            if (!pblocktree->WriteKeyImage(vchImage, spentKeyImage))
    ...
    try
    {
        vchEphemPK.resize(EC_COMPRESSED_SIZE);
    }
    ...
    for (uint32_t i = 0; i < tx.vout.size(); ++i)
    {
        ...
        if (pblocktree->ReadAnonOutput(pkCoin, ao)) // check if exists
        {
            if (false == blockHash.IsNull())
            {
                if (fDebugRingSig)
        ...
        if (!pblocktree->WriteAnonOutput(pkCoin, ao))
        {
        ...
        if (!fOwnOutput)
        {
            for (mi = mapExtAccounts.begin(); mi != mapExtAccounts.end(); ++mi)
            {
                ...
                for (AccStealthKeyMap::iterator it = ea->mapStealthKeys.begin(); it != ea->mapStealthKeys.end(); ++it)
                {
                    ...
                    CPubKey scndcpkE(pkExtracted);
                    if (!scndcpkE.IsValid() || scndcpkE != pkCoin)
                    {
                        continue;
                    }
        ...
        if (!fHaveSpendKey)
        {
            ...
            if (!wdb.WriteLockedAnonOutput(ckCoinId, lockedAo))
            {
        ...
        else
        {
            ...
            CKey ckey;
            ckey.Set(&sSpendR.e[0], &sSpendR.e[32], true);
            ...
            if (!AddKeyPubKey(ckey, ckey.GetPubKey()))
            {
            ...
            ec_point pkImage;
            ec_point pkOldImage;
            RingSignatureMgr::GetInstance().getOldKeyImage(pkCoin, pkOldImage);
            if (RingSignatureMgr::GetInstance().generateKeyImage(pkTestSpendR, sSpendR, pkImage) != 0)
            {
            ...
            fSpentAOut = GetKeyImage(*pblocktree, pkImage, kis, fInMemPool) ||
                         GetKeyImage(*pblocktree, pkOldImage, kis, fInMemPool);

            COwnedAnonOutput oao(outpoint, fSpentAOut);

            if (!wdb.WriteOwnedAnonOutput(pkImage, oao)      ||
                !wdb.WriteOldOutputLink(pkOldImage, pkImage) ||
                !wdb.WriteOwnedAnonOutputLink(pkCoin, pkImage))
            {
...
int CWallet::ExtKeySaveKey(CExtKeyAccount *sea, const CKeyID &keyId, CEKASCKey &asck) const
{
    ...
    {
        return errorN(1, "%s SaveKey failed.");
    }

    bool fUpdateAcc;
    WalletBatch wdb{*database};

    if (0 != ExtKeyAppendToPack(sea, keyId, asck, fUpdateAcc))
    {
        return errorN(1, "%s ExtKeyAppendToPack failed.");
    }

    // only neccessary if nPackStealth has changed
    //
    if (fUpdateAcc)
    {
        ...
        {
            return errorN(1, "%s WriteExtAccount failed.");
        }
    }
...
int CWallet::ExtKeyAppendToPack(CExtKeyAccount *sea, const CKeyID &idKey, CEKASCKey &asck, bool &fUpdateAcc) const
{
    ...
    if (!wdb.ReadExtStealthKeyChildPack(idAccount, sea->nPackStealthKeys, asckPak))
    {
        // new pack
        //
        asckPak.clear();
    ...
    try
    {
        asckPak.push_back(CEKASCKeyPack(idKey, asck));
    }
    catch (std::exception& e)
    {
        return errorN(1, "push_back failed.");
    }

    if (!wdb.WriteExtStealthKeyChildPack(idAccount, sea->nPackStealthKeys, asckPak))
    {
        return errorN(1, "Save key pack failed.");
    }
...
bool CWallet::FindStealthTransactions(const CTransaction& tx, mapValue_t& mapNarr)
{
    ...
    for (const auto& txout : tx.vout)
    {
        ...
        //
        if (tx.IsAnon() && txout.IsAnonOutput())
        {
            continue;
        }
        ...
        else if (!txout.scriptPubKey.GetOp(itTxA, opCode, vchEphemPK) || vchEphemPK.size() != EC_COMPRESSED_SIZE)
        {
        ...
        for (const auto& txoutB : tx.vout)
        {
            ...
            for (it = stealthAddresses.begin(); it != stealthAddresses.end(); ++it)
            {
                ...
                if (IsLocked())
                {
                    ...
                    WalletBatch wdb{*database};
                    if (!wdb.WriteStealthKeyMeta(keyId, lockedSkMeta))
                    {
            ...
            for (mi = mapExtAccounts.begin(); mi != mapExtAccounts.end(); ++mi)
            {
                ...
                for (auto it = ea->mapStealthKeys.begin(); it != ea->mapStealthKeys.end(); ++it)
                {
                    const CEKAStealthKey &aks = it->second;
    ...
    const CTransaction& tx = *ptx;
    {
        ...
        if (!tx.IsCoinBase() && !tx.IsCoinStake())
        {
            ...
            if (tx.IsAnon())
            {
                ...
                if (!ProcessAnonTransaction(tx, blockHash, fIsMine, mapNarr))
                {
                ...
                {
                    // TODO TSB: check if we ever need this NotifyTransactionChanged and
                    //           update vUpdatedTxns in ProcessAnonTx as well if that's the case
                    //           also, remove the log

                    // TODO TSB again: NotifyTransactionChanged exists and is called as expected, just implement the vUpdateTxns stuff!

                    //LogPrintf("ProcessAnonTransaction returned TRUE for txhash = %s\n", tx.GetHash().ToString().c_str());
...
void CWallet::ReacceptWalletTransactions()
{
    ...
    for (const std::pair<const int64_t, CWalletTx*>& item : mapSorted) {
        ...
        wtx.AcceptToMemoryPool(*locked_chain, 0 /*maxTxFee*/, state);
    }
}
...
bool CWalletTx::RelayWalletTransaction(interfaces::Chain::Lock& locked_chain, CConnman* connman)
{
    ...
    if (!IsCoinBase() && !isAbandoned() && GetDepthInMainChain(locked_chain) == 0)
    {
        ...
        if (InMempool() || AcceptToMemoryPool(locked_chain, 0 /*maxTxFee*/, state)) {
                                 ...
                                 std::set<CInputCoin>& setCoinsRet, CAmount& nValueRet/*, const CoinSelectionParams& coin_selection_params, bool& bnb_used*/) const
{
    ...
    if (coin_selection_params.use_bnb) {
        ...
        bnb_used = true;
        return SelectCoinsBnB(utxo_pool, nTargetValue, 0, setCoinsRet, nValueRet, 0);
    } else {*/
...
bool CWallet::CreateTransaction(interfaces::Chain::Lock& locked_chain, const std::vector<CRecipient>& vecSend, CTransactionRef& tx, CReserveKey& reservekey, CAmount& nFeeRet,
                         int& nChangePos, std::string& strFailReason, const CCoinControl& coin_control, bool sign)
{
    CAmount nValue = 0;
    ...
    for (const auto& recipient : vecSend)
    {
        ...
        {
            strFailReason = _("Transaction amounts must not be negative nor zero");
            return false;
        }
        ...
        LOCK(cs_wallet);
        {
            ...
            */
            ...
            else
            {
                ...
                const auto C_CHANGE_TYPE{OutputType::LEGACY};
                LearnRelatedScripts(vchPubKey, C_CHANGE_TYPE);
                scriptChange = GetScriptForDestination(GetDestinationForKey(vchPubKey, C_CHANGE_TYPE));
            }
            ...
            while (true)
            {
                ...
                int64_t nValueToSelect{nValue};
                if (0 == nSubtractFeeFromAmount)
                {
                ...
                {
                    // TODO TSB: check with business if this is OK
                    // change < 0.01 => simply move all change to fees
                    //
                    if (nChange < CENT)
                    {
                        nFeeRet = nChange;
                ...
                if (nChange > 0)
                {
                    // insert change at random position
                    //
                    nChangePos = GetRandInt(txNew.vout.size() + 1); // + 1 is the upcoming change output

                    // don't put change output between value and narration outputs
                    // (first or last positions are fine though)
                    //
                    if (nChangePos > 0 && nChangePos < txNew.vout.size() + 1) // + 1 is the upcoming change output
                    {
                        while (nChangePos > 0 && txNew.vout[nChangePos].nValue != 0)
                        {
                            --nChangePos;
                        }
                    }
                    ...
                    //
                    txNew.vout.emplace(txNew.vout.begin() + nChangePos, nChange, scriptChange);
                }
                else
                {
                    nChangePos = -1;
                ...
                {
                    txNew.vin.emplace_back(coin.outpoint);
                }

                // TODO TSB: move the sign part outside of the while true: careful with fee calculation, depends on sz
                //
                if (sign)
                {
                    int nIn = 0;
                ...
                // limit size
                //
                auto txSize{::GetSerializeSize(txNew, PROTOCOL_VERSION)};
                if (txSize > MAX_STANDARD_TX_WEIGHT)
                {
                ...
                //
                int64_t nPayFee = MIN_TX_FEE * (1 + txSize / 1000);

                // TODO TSB: avoid copying everything in CTransaction from CMutableTransaction for GetMinFee
                //
                int64_t nMinFee = CTransaction{txNew}.GetMinFee(1, txSize);

                auto maxFee{std::max(nPayFee, nMinFee)};
                if (nFeeRet < maxFee)
                {
                    nFeeRet = maxFee;
                    continue;
...
{
    {
        ...
        {
            // Broadcast
            if (!wtx.AcceptToMemoryPool(*locked_chain, 0 /*maxTxFee*/, state)) {
...
int CMerkleTx::GetBlocksToMaturity(interfaces::Chain::Lock& locked_chain) const
{
    ...
    {
        return std::max(0, static_cast<int>(Params().GetConsensus().nCoinstakeMaturity + 1) - chain_depth);
    }
    else
    {
        assert(0);

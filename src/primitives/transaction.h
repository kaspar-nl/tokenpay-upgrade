...
public:
    ...
    {
        return (scriptSig.size() >= (2 + (33 + 32 + 32)) // op_return + op_anon + ?!
    ...
    void ExtractKeyImage(ec_point& kiOut) const
    {
        ...
        std::memcpy(&kiOut[0], prevout.hash.begin(), EC_SECRET_SIZE);
        kiOut[EC_SECRET_SIZE] = prevout.n & 0xFF;
    }
};
...
public:
    ...
    bool IsEmpty() const
    {
        return 0 == nValue && scriptPubKey.empty();
    }
...
public:
    ...
    bool IsAnon() const
    {
        return ANON_TXN_VERSION == nVersion;
    }

    bool HasStealthOutput() const;

    int64_t GetMinFee(uint32_t blockSize = 1, uint32_t txSize = 0) const;

...
std::size_t nOrphanBlocksSize = 0;
 ...
 * @return random value
 */
uint32_t insecure_rand_Rz{0};
uint32_t insecure_rand_Rw{0};
...
{
    const uint256* prevHash = &hash;
    const uint256* currentHash = &hash;

    do
    {
        auto itr = mapOrphanBlocks.find(*currentHash);
        if (mapOrphanBlocks.cend() == itr)
        {
            return *prevHash;
        }

        prevHash = currentHash;
        currentHash = &itr->second->hashPrev;
    }
    while (true);
...
const COrphanBlock* AddOrphanBlock(const CBlock* pblock)
{
    ...
    mapOrphanBlocks.insert(std::make_pair(pblock->GetHash(), orphan));
    mapOrphanBlocksByPrev.insert(std::make_pair(orphan->hashPrev, orphan));
...
namespace
{
    using BlockPtrList = std::vector<std::shared_ptr<const CBlock>>;
}
...
static void PushNodeVersion(CNode *pnode, CConnman* connman, int64_t nTime)
{
    ...
    connman->PushMessage(pnode, CNetMsgMaker(INIT_PROTO_VERSION).Make(NetMsgType::VERSION, PROTOCOL_VERSION, (uint64_t)nLocalNodeServices, nTime, addrYou, addrMe,
            nonce, strSubVersion, nNodeStartingHeight, nIsFullNode));
...
bool static ProcessMultiblockMessage(const BlockPtrList& iBlockPtrList, CNode* ipFrom, CConnman* ipConnman, const CChainParams& iChainParams)
{
    assert(false == iBlockPtrList.empty() && iBlockPtrList.size() <= MAX_MULTI_BLOCK_ELEMENTS);
    assert(ipFrom);

    uint256 lastNewBlockHash;
    for (const auto& blockPtrItr : iBlockPtrList)
    {
        ...
        LogPrint(BCLog::NET, "processing block %s, peer=%d\n", blockPtrItr->GetHash().ToString(), ipFrom->GetId());

        bool fNewBlock{false};
        bool forceProcessing{false};
        const uint256 hash{blockPtrItr->GetHash()};

        {
            ...
            //
            mapBlockSource.emplace(hash, std::make_pair(ipFrom->GetId(), true));
        }

        {
            LOCK(cs_main);
            if (!mapBlockIndex.count(blockPtrItr->hashPrevBlock))
            {
                LogPrintf("Got orphan=%s, prev=%s, orphans_count=%d\n\n\n", blockPtrItr->GetHash().ToString(), blockPtrItr->hashPrevBlock.ToString(), mapOrphanBlocks.size());

                if (blockPtrItr->IsProofOfStake())
                {
                    // Limited duplicity on stake: prevents block flood attack
                    // Duplicate stake allowed only when there is orphan child block

                    if (setStakeSeenOrphan.count(blockPtrItr->GetProofOfStake()) && !mapOrphanBlocksByPrev.count(hash))
                        return error("ProcessBlock() : duplicate proof-of-stake (%s, %d) for orphan block %s", blockPtrItr->GetProofOfStake().first.ToString(), blockPtrItr->GetProofOfStake().second, hash.ToString());
                }
                PruneOrphanBlocks();
                const COrphanBlock* orphan = AddOrphanBlock(blockPtrItr.get());

                // Ask this guy to fill in what we're missing
                auto hint = chainActive.Tip();
                if (hint)
                {
                    hint = hint->pprev;
                }
                ...
                auto blockLocator = chainActive.GetLocator(hint);
                uint256* from = nullptr;
                if (!blockLocator.vHave.empty())
                {
                    from = &blockLocator.vHave.front();
                }
                LogPrintf("Asking orphan provider from=%s to=%s\n", from ? from->ToString() : "null", GetOrphanRoot(hash).ToString());
                ...
                ipConnman->PushMessage(ipFrom,
                                       CNetMsgMaker(ipFrom->GetSendVersion()).Make(
                                       NetMsgType::GETBLOCKS,
                                       chainActive.GetLocator(hint),
                                       GetOrphanRoot(hash)));
                ...
                {
                    ipFrom->AskFor(CInv(MSG_BLOCK, WantedByOrphan(orphan)));
                }
        ...
        if (ProcessNewBlock(iChainParams, blockPtrItr, forceProcessing, &fNewBlock))
        {
            LogPrint(BCLog::NET, "ProcessMultiBlockMessage: processblock returned TRUE\n");

            if (fNewBlock)
            {
                ipFrom->nLastBlockTime = GetTime();
                lastNewBlockHash = hash;
            }
            else
            {
                LOCK(cs_main);
            ...
            LogPrint(BCLog::NET, "Recursively process orphans \n");
            ...
            for (unsigned int i = 0; i < vWorkQueue.size(); i++)
            {
                ...
                for (auto mi = mapOrphanBlocksByPrev.lower_bound(hashPrev); mi != mapOrphanBlocksByPrev.upper_bound(hashPrev); ++mi)
                {
                    ...
                    {
                        CDataStream ss(mi->second->vchBlock, SER_DISK, 0);
                        ss >> *block;
                    }

                    bool newBlock{false};
                    if (ProcessNewBlock(iChainParams, block, true, &newBlock))
                    {
                        lastNewBlockHash = mi->second->hashBlock;
    ...
    if (!lastNewBlockHash.IsNull())
    {
        auto mapBlockIndexFindItr = mapBlockIndex.find(lastNewBlockHash);
        assert(mapBlockIndex.cend() != mapBlockIndexFindItr);

        LogPrint(BCLog::NET, "Finished ProcessNewBlock, lastblock=%s, h=%d, asking for more now\n", lastNewBlockHash.ToString(), mapBlockIndexFindItr->second->nHeight);

        ipConnman->PushMessage(ipFrom,
                               CNetMsgMaker(ipFrom->GetSendVersion()).Make(NetMsgType::GETBLOCKS,
                                                                           chainActive.GetLocator(mapBlockIndexFindItr->second),
                                                                           uint256()));
    }
...
bool static ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, int64_t nTimeReceived, const CChainParams& chainparams, CConnman* connman, const std::atomic<bool>& interruptMsgProc, bool enable_bip61)
{
    ...
    if (strCommand == NetMsgType::VERSION) {
        ...
        // TokenPay: do not send the final alert at all since old peers don't know about it
        /*
    ...
    if (strCommand == NetMsgType::INV) {
        ...
        for (CInv &inv : vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                {
                    pfrom->AskFor(inv);
                    LogPrint(BCLog::NET, "asking block=%s from peer=%d\n", inv.hash.ToString(), pfrom->GetId());
                }
                ...
                if (!fImporting && !fReindex)
                {
                    if (mapOrphanBlocks.count(inv.hash))
                    {
                        ...
                        connman->PushMessage(pfrom,
                                             CNetMsgMaker(pfrom->GetSendVersion()).Make(NetMsgType::GETBLOCKS,
                                                                                        chainActive.GetLocator(hint),
                                                                                        uint256()));
                    }
    ...
    if (strCommand == NetMsgType::HEADERS && !fImporting && !fReindex) // Ignore headers received while importing
    {
        ...
        for (unsigned int n = 0; n < nCount; n++) {
            ...
            ReadCompactSize(vRecv); // ignore vchblocksig count
        }
    ...
    if (strCommand == NetMsgType::MBLK && !fImporting && !fReindex)  // Ignore blocks received while importing
    {
        BlockPtrList blockPtrList;
        vRecv >> blockPtrList;

        if (blockPtrList.empty() || blockPtrList.size() > MAX_MULTI_BLOCK_ELEMENTS)
        {
            LOCK(cs_main);
            Misbehaving(pfrom->GetId(), 10, strprintf("Wrong blocks count in mblk: %u\n", blockPtrList.size()));
            return false;
        }

        LogPrint(BCLog::NET, "received mblk with size =  %d\n", blockPtrList.size());
        return ProcessMultiblockMessage(blockPtrList, pfrom, connman, chainparams);
    }
    ...
    const Consensus::Params& consensusParams = Params().GetConsensus();
    {
        ...
        if (pingSend) {
            ...
            if (pto->nVersion > BIP0031_VERSION) {
                ...
                connman->PushMessage(pto, msgMaker.Make(NetMsgType::PING, nonce, connman->GetBestHeight()));
            } else {
        ...
        if (!state.fSyncStarted && !pto->fClient && !fImporting && !fReindex) {
            ...
            if ((nSyncStarted == 0 && fFetch) || pindexBestHeader->GetBlockTime() > GetAdjustedTime() - 24 * 60 * 60) {
                ...
                // TODO TSB
                LogPrintf("TokenPay: initial blockchain download has started\n");
            }
        }
        ...
        if (!pto->fClient && ((fFetch && !pto->m_limited_node) || !IsInitialBlockDownload()) && state.nBlocksInFlight < MAX_BLOCKS_IN_TRANSIT_PER_PEER) {
            ...
            for (const CBlockIndex *pindex : vToDownload) {
                ...
                LogPrint(BCLog::NET, "Sending getdata block=%s, h=%d, peer=%d\n", pindex->GetBlockHash().ToString(),
        ...
        if (pto->nVersion >= FEEFILTER_VERSION && gArgs.GetBoolArg("-feefilter", DEFAULT_FEEFILTER) &&
            ...
            if (timeNow > pto->nextSendTimeFeeFilter) {
                static CFeeRate default_feerate(DEFAULT_MIN_TX_FEE);

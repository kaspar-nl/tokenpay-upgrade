...
static const int C_SOCKS_PORT{9090};

void InitTor()
{
    TorConfigurationOptions options{};
    options.ignoreMissingConfigFile();
    options.setSocksPort(C_SOCKS_PORT);
    options.setHiddenServicePort(Params().GetDefaultPort());
    options.setHiddenServiceVersion(2);

    auto dataFolder{GetDataDir() / "tor"};
    fs::create_directory(dataFolder);
    options.setDataFolder(dataFolder);

    auto hiddenServiceFolder{dataFolder / "hs"};
    fs::create_directory(hiddenServiceFolder);
    options.setHiddenServiceFolder(hiddenServiceFolder);

    auto logFile{dataFolder / "tor.log"};
    options.setLogFile(logFile, TorConfigurationOptions::LogSeverity::E_NOTICE);

    threadGroup.create_thread(std::bind(&TraceThread<std::function<void()>>,
                              "torthread",
                              std::function<void()>(std::bind(&TorMgr::run,
                                                    std::ref(TorMgr::GetInstance()),
                                                    std::move(options)))));
}

bool ConnectToTor()
{
    LogPrintf("Bringing up Tor SOCKS server...\n");

    if (false == TorMgr::GetInstance().isRunning())
    {
        LogPrintf("Unable to bring up Tor SOCKS server\n");
        return false;
    }

    while (false == TorMgr::GetInstance().waitWithTimeoutToBringSocksServerUp(std::chrono::milliseconds{100}))
    {
        if (ShutdownRequested())
        {
            LogPrintf("Shutdown requested, canceling Tor SOCKS server bring up\n");
            return false;
        }
    }
    ...
    CService socksServerAddress;
    if (!Lookup("localhost", socksServerAddress, C_SOCKS_PORT, fNameLookup))
    {
        LogPrintf("Unable to look up 'localhost:%d'\n", C_SOCKS_PORT);
        return false;
    }

    proxyType socksServerProxy = proxyType(socksServerAddress, true);
    if (!socksServerProxy.IsValid())
    {
        LogPrintf("Invalid proxy 'localhost:%d'\n", C_SOCKS_PORT);
        return false;
    }

    SetProxy(NET_IPV4, socksServerProxy);
    SetProxy(NET_IPV6, socksServerProxy);
    SetProxy(NET_ONION, socksServerProxy);

    SetNameProxy(socksServerProxy);
    SetReachable(NET_ONION, true);

    boost::filesystem::path onionHostnamePath = GetDataDir() / "tor" / "hs" / "hostname";
    if (false == boost::filesystem::exists(onionHostnamePath))
    {
        LogPrintf("Tor hostname file does not exist\n");
        return false;
    }

    std::string onionHostname;
    CService onionHostnameAddress;
    std::ifstream onionHostnameFileStream(onionHostnamePath.string());

    onionHostnameFileStream >> onionHostname;
    if (false == Lookup(onionHostname.c_str(), onionHostnameAddress, GetListenPort(), fNameLookup) ||
        false == onionHostnameAddress.IsValid())
    {
        LogPrintf("Invalid Tor hostname '%s'\n", onionHostname.c_str());
        return false;
    }

    AddLocal(onionHostnameAddress, LOCAL_MANUAL);

    LogPrintf("Connection to Tor via SOCKS successfully established\n");
    return true;
}

bool WaitToBuildTorCircuit()
{
    LogPrintf("Setting up a Tor circuit...\n");

    if (false == TorMgr::GetInstance().isRunning())
    {
        LogPrintf("Unable to build a Tor circuit\n");
        return false;
    }

    while (false == TorMgr::GetInstance().waitWithTimeoutToBuildCircuit(std::chrono::milliseconds{100}))
    {
        if (ShutdownRequested())
        {
            return false;
    ...
    LogPrintf("Tor circuit built successfully\n");
    return true;
}
...
void Interrupt()
{
    ...
    if (TorMgr::GetInstance().isRunning())
    {
        TorMgr::GetInstance().shutdown();
    }
}
...
void SetupServerArgs()
{
    ...
    gArgs.AddArg("-port=<port>", strprintf("Listen for connections on <port> (default: %u, testnet: %u, regtest: N/A)", defaultChainParams->GetDefaultPort(), testnetChainParams->GetDefaultPort()), false, OptionsCategory::CONNECTION);
    ...
    gArgs.AddArg("-checkblockindex", strprintf("Do a full consistency check for mapBlockIndex, setBlockIndexCandidates, chainActive and mapBlocksUnlinked occasionally. (default: %u, regtest: N/A)", defaultChainParams->DefaultConsistencyChecks()), true, OptionsCategory::DEBUG_TEST);
    gArgs.AddArg("-checkmempool=<n>", strprintf("Run checks every <n> transactions (default: %u, regtest: N/A)", defaultChainParams->DefaultConsistencyChecks()), true, OptionsCategory::DEBUG_TEST);
    ...
    //gArgs.AddArg("-minrelaytxfee=<amt>", strprintf("Fees (in %s/kB) smaller than this are considered zero fee for relaying, mining and transaction creation (default: %s)",
      //  CURRENCY_UNIT, FormatMoney(DEFAULT_MIN_TX_FEE)), false, OptionsCategory::NODE_RELAY);
    ...
    gArgs.AddArg("-rpcport=<port>", strprintf("Listen for JSON-RPC connections on <port> (default: %u, testnet: %u, regtest: N/A)", defaultBaseParams->RPCPort(), testnetBaseParams->RPCPort()), false, OptionsCategory::RPC);
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    // TokenPay: wait for Tor SOCKS connectivity
    //
    if (false == ConnectToTor())
    {
        return false;
    }

    // TokenPay: disable all proxy/onion/listen/externalip stuff
    /*
    ...
    */
    ...
    // TokenPay: TorControl no longer used in TokenPay
    /*
    ...
    */
    ...
    // TokenPay: wait for Tor circuit before starting connman
    //
    if (false == WaitToBuildTorCircuit())
    {
        return false;
    }

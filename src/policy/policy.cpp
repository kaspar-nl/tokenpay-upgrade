...
{
    if (false == tx.IsAnon() && (tx.nVersion > CTransaction::MAX_STANDARD_VERSION || tx.nVersion < 1)) {
        reason = "version";
        return false;
    }
    ...
    for (const CTxOut& txout : tx.vout) {
        ...
        {
            if (txout.nValue < 1 || txout.scriptPubKey.size() > MIN_ANON_OUT_SIZE + MAX_ANON_NARRATION_SIZE)
            {
    ...
    // only one OP_RETURN per output is permitted
    if (nDataOut > nTxnOut) {
